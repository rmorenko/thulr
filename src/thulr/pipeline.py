...
the shapes an answer comes back in — lives in `thulr.run`, and is
...
from thulr.ingest.commits import blame_links, blame_map, commit_chunks, read_commits
from thulr.ingest.link_extract import links_for, occurrences_of
from thulr.ingest.manifests import read_manifests
from thulr.links import Edge, Link, LinkKind, LinkStore
from thulr.model import Chunk, Hit, Kind, SearchFilter, SourceFile
from thulr.rank.reranker import Reranker
from thulr.rewrite.rewriter import Rewriter
from thulr.run import (
    _WRITE_BATCH,
    Authorship,
    Definition,
    FileReport,
...
from thulr.store import VectorStore
...
"""Silent unless somebody attaches a handler; `thulr serve` does.
...
class Pipeline:
    ...
    Attributes:
        ...
        state_dir: Where `index` keeps the per-repo "last indexed commit"
            ...
            uri from there (see `thulr.cli.build_pipeline`).
        ...
        stats: Where searches are recorded, or None to record nothing.
            Local by construction and by rule — see `thulr.stats`.
    ...
    def index(self, *, progress: Callable[[str], None] | None = None) -> IndexReport:
        ...
        for repo in config.repos:
            ...
            # is the caller's business (see `thulr.ui`). A plain
    ...
    def why(self, target: str, *, limit: int = 3) -> list[Definition]:
        ...
        itself — `thulr why` and the MCP tool — from the same three
    ...
    def describe(self, path: Path) -> FileReport | None:
        ...
        `thulr explain` in library terms. It used to do this itself,
        which cost the CLI eight imports out of `thulr.ingest` and put
    ...
    def search(
        ...
        *,
        ...
        search it always was. See `thulr.rewrite` for what that buys
        and what it sends.
...
__all__ = [
    # Re-exported from `thulr.run`, which is where they live now: every
