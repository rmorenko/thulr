...
class Document:
    ...
    Attributes:
        ...
        text: The document body. Markdown where the source speaks it,
            ...
            is `thulr.snapshot`'s job, not the fetch's.
...
"""Connector implementations shipped with thulr, by `type` name.
...
def _ensure_plugins() -> None:
    ...
    not work. A plugin must `from thulr.connectors import Connector` at
    ...
    with _plugin_lock:
        ...
        from thulr.connectors.plugins import load_connectors
...
from thulr.connectors.github import GitHubConnector  # noqa: E402
from thulr.connectors.http import GenericHttpConnector  # noqa: E402
...
with what is installed, and this is "what does thulr itself claim",
which does not. A plugin may not take one of these — see
`thulr.connectors.plugins`."""
