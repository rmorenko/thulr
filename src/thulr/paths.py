...
2. Workspace mode — walk from CWD up to `/` for a `thulr.toml`,
   like `git` finds `.git/`. Index lives in `.thulr/` next to the
   config: the natural place for a per-project tool.
3. User mode — `$XDG_CONFIG_HOME/thulr/config.toml` via platformdirs.
   Index lives in `$XDG_DATA_HOME/thulr/<workspace-name>/`: for
   `pipx install` and other from-anywhere invocations.
4. System mode — `$XDG_CONFIG_DIRS/thulr/config.toml`. Read-only
...
APP_NAME = "thulr"
CONFIG_FILE = "thulr.toml"
USER_CONFIG_FILE = "config.toml"
WORKSPACE_INDEX_DIR = ".thulr"
ENV_OVERRIDE = "THULR_CONFIG"
...
def find_config(cwd: Path | None = None) -> ConfigLocation | None:
    """Locate a thulr config, or None if there is none anywhere.

    An explicit `$THULR_CONFIG` that points at a non-existent file
...
def searched_paths(cwd: Path | None = None) -> list[str]:
    ...
    that tells the user exactly where thulr looked.
    """
    override = os.environ.get(ENV_OVERRIDE)
    start = cwd or Path.cwd()
    return [
        f"$THULR_CONFIG ({override or 'not set'})",
...
def workspace_id(config_path: Path, workspace_name: str) -> str:
    ...
    Used as the subdirectory name under `$XDG_DATA_HOME/thulr/` in
    ...
    stays as a human-readable prefix so `ls ~/.local/share/thulr/`
...
def resolve_index_dir(location: ConfigLocation, workspace_name: str) -> Path:
    ...
    Workspace/override configs put the index in `.thulr/` next to
    the config — the per-project convention, isolated by directory.
    User/system configs push the index to `$XDG_DATA_HOME/thulr/
...
def workspace_config_path(cwd: Path | None = None) -> Path:
    """Target of `thulr init` (workspace mode): `./thulr.toml`."""
    return (cwd or Path.cwd()) / CONFIG_FILE


def user_config_file() -> Path:
    """Target of `thulr init --user`: `$XDG_CONFIG_HOME/thulr/config.toml`."""
    return user_config_path(APP_NAME) / USER_CONFIG_FILE


def resolve_cache_dir() -> Path:
    """Root of thulr-owned cache: `$XDG_CACHE_HOME/thulr/`.
    ...
    any time without breaking thulr — the next run will re-download
    what it needs.
    """
