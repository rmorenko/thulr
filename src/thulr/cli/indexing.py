...
from thulr.ingest import GitCommandError, NotAGitRepositoryError, sync_repo
from thulr.snapshot import materialize
from thulr.ui import progress as ui_progress
...
def index() -> None:
    ...
    if report.unparsed:
        ...
        typer.echo(
            ...
            + " (indexed as text; `thulr explain <path>` for one of them)",
            err=True,
        )
    ...
    if report.mostly_unclaimed:
        ...
        typer.echo(
            ...
            "`thulr explain <path>` says which rule left a file out)",
            err=True,
        )
    if pipeline.links is not None:
        ...
        if drift:
            ...
            # enough to send someone looking. Listing them is `thulr
            # refs`/`why` territory.
            first = drift[0]
            typer.echo(
...
def sync(
    ...
    if not pulled and not snapshots:
        typer.echo(
            "nothing to sync — give a repo a remote (`thulr add-repo <id> <path> --remote "
