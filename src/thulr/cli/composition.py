...
from thulr.config import Backend, Config, LinksBackend, Provider
from thulr.embed import Embedder, FakeEmbedder, SentenceTransformerEmbedder
from thulr.embed.remote import RemoteEmbedder
from thulr.links import LinkStore
from thulr.paths import ConfigLocation, make_index_dir, resolve_cache_dir, searched_paths
...
from thulr.rewrite.remote import RemoteRewriter
...
def config_or_default() -> Config:
    ...
    try:
        ...
        typer.echo(f"error: cannot read the thulr config — {exc}", err=True)
        raise typer.Exit(code=1) from exc
    if config.is_default:
        typer.echo("warning: no thulr config found. Checked:", err=True)
...
def require_config_file(config: Config) -> ConfigLocation:
    ...
    built-in defaults (see `thulr.config`). That is enough for `status`,
    ...
    The warning already listed where thulr looked; this only adds the
    way out.
    """
    location = config.location
    if location is None:
        typer.echo(
            "error: this command needs a config file — run `thulr init <name>` "
            "(workspace) or `thulr init --user <name>` (user).",
            err=True,
        )
