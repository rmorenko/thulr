...
human-edited `thulr.toml`, read on every command, and the index
...
`THULR_DEBUG=1` opens a door: the traceback comes through, the
...
from thulr.cli.composition import build_pipeline, build_store
from thulr.cli.external import fetch
from thulr.cli.indexing import compact, index, sync
from thulr.cli.interfaces import mcp, serve, shell
from thulr.cli.searching import refs, search, why
from thulr.cli.workspace import (
    add_repo,
    deps,
    domains,
    dupes,
...
DEBUG_ENV = "THULR_DEBUG"
"""Set to any non-empty value to trade the tidy output for the whole
truth. Named like `THULR_PLAIN`, and for the same reason: the one
...
@app.callback()
def main() -> None:
    """thulr — semantic search across the repositories of a workspace."""
    # One thulr process serves exactly one command, so `Config()` loads
...
def run() -> None:
    ...
    With `THULR_DEBUG` set, nothing is caught at all: a RuntimeError
...
def _open_the_door() -> None:
    ...
    logging.getLogger("thulr").setLevel(logging.DEBUG)
...
def _advice(exc: RuntimeError) -> str:
    ...
    return (
        ...
        "`thulr index` (ids are deterministic, re-indexing is cheap)"
    )
