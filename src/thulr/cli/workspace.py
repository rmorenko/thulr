...
from thulr.config import Backend, Config, LinksBackend, Provider, Repository, RepoSource
from thulr.deps import analyse
from thulr.ingest import SKIP_REASONS
from thulr.ingest.git_state import STATE_FILE, IndexState
from thulr.paths import user_config_file, workspace_config_path
from thulr.stats import SearchLog

if TYPE_CHECKING:  # pragma: no cover - annotations only
    from thulr.domains import Domains
...
def init(
    ...
    user: Annotated[
        bool,
        typer.Option("--user", help="Install into $XDG_CONFIG_HOME/thulr/ instead of the CWD"),
    ] = False,
    force: Annotated[bool, typer.Option("--force", help="Overwrite an existing config")] = False,
) -> None:
    """Create a thulr config.

    Two modes: workspace (default) writes `./thulr.toml`, so the tool
    lives with the project (like `git init`); `--user` writes
    `$XDG_CONFIG_HOME/thulr/config.toml`, for `pipx install` and other
...
def add_repo(
    ...
    remote: Annotated[
        str | None,
        typer.Option("--remote", help="Clone url; `thulr sync` keeps <path> up to date from it"),
    ] = None,
    source: Annotated[
        RepoSource | None,
    ...
    `thulr sync` clones it if `path` does not exist yet and
    ...
    suffix table and lives in `thulr.toml` rather than on a command
    line.
    """
...
def status() -> None:
    ...
    if not config.repos:
        typer.echo("repos: none — add one with `thulr add-repo <id> <path>`")
        return
    typer.echo("repos:")
...
def _indexed_at(config: Config, repo_id: str) -> str:
    ...
    visible in `thulr.toml`. Nothing said whether the thing the tool
...
def domains(
    ...
    **Meaning**, from the vectors already in the index: a file's subject
    ...
    """
    from thulr.domains import analyse
...
def _too_few(found: Domains, *, pipeline: Pipeline, chosen: str, prefix: str | None) -> None:
    ...
    if found.files:
        ...
        typer.echo(
            f"no indexed code in {chosen} — `thulr index` if it is new, "
            "or `thulr explain <path>` to see which rule left its files out"
        )
...
def dupes(
    ...
    against this and lost; see `thulr.dupes` for the numbers.
