...
def shell() -> None:
    ...
    try:
        from thulr import shell as shell_module
...
def mcp() -> None:
    ...
    A workspace already running `thulr serve` can offer the same tools
...
def serve(
    ...
    typer.echo(f"thulr '{config.name}' on http://{host}:{port}  (admin at /admin)")
    ...
    logging.getLogger("thulr").setLevel(logging.INFO)
