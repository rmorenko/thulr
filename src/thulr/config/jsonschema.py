"""A JSON Schema for `thulr.toml`, derived from the rules that enforce it.
...
`thulr.config.validate` or the enums in `thulr.config.schema`, so a
...
The generated document lives at `thulr.schema.json` in the repository
...
from thulr.config.validate import REPO_KEYS
from thulr.model import Kind

SCHEMA_FILE = "thulr.schema.json"
"""Where the generated document is kept, relative to the repository root."""

SCHEMA_ID = "https://raw.githubusercontent.com/rmorenko/thulr/main/thulr.schema.json"
...
def _repo_properties() -> dict[str, Any]:
    ...
    return {
        ...
        "remote": {
            "type": "string",
            "description": "Clone url; `thulr sync` keeps `path` current from it.",
        },
        "source": {
            "type": "string",
    ...
    return _section(
        {
            "uri": {
                ...
                "Defaults to `.thulr/` beside this file.",
            },
    ...
    return _section(
        {
            "enabled": {
                "type": "boolean",
                "default": True,
                "description": "Record searches locally for `thulr stats`. "
...
def build() -> dict[str, Any]:
    ...
    Returns:
        A draft 2020-12 document describing `thulr.toml`.
    ...
    return {
        ...
        "$id": SCHEMA_ID,
        "title": "thulr workspace configuration",
        "type": "object",
