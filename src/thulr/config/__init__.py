"""Workspace configuration: the `thulr.toml` file, as one live object.
...
from thulr.config.document import render, repo_entry
from thulr.config.schema import (
    DEFAULT_RANK_MODEL,
...
from thulr.config.validate import validate, validate_repo
from thulr.connectors import ConnectorSpec
from thulr.model import Kind
from thulr.paths import ConfigLocation, Mode, find_config, resolve_index_dir


class Config:
    """The workspace config, as one process-wide instance.

    Holds the parsed `thulr.toml` document in `_data` and reads every
    ...
    in-memory default (that is what `thulr init` needs), `Config.reset()`
    ...
    DEFAULT: ClassVar[dict[str, Any]] = {
        ...
        # (see `store_uri`). Baking `.thulr` in would freeze a
    ...
    def __init__(self, path: Path | None = None) -> None:
        ...
        if hasattr(self, "_data"):
            return
        if path is not None:
            # Same mode an explicit `$THULR_CONFIG` gets: the caller named
    ...
    def default(
        ...
        *,
        ...
        What `thulr init` needs: project defaults for a workspace that has
    ...
    def index_dir(self) -> Path:
        ...
        `thulr.paths.resolve_index_dir`): `.thulr/` next to a workspace
        config, `$XDG_DATA_HOME/thulr/<id>/` for a user or system one.
    ...
    def _setting(self, section: str, key: str, default: Any = None) -> Any:
        ...
        raw document stays the source of truth: a section thulr does
    ...
    def store_uri(self) -> str:
        ...
        must not drop a `.thulr/` into whatever directory the user
    ...
    def rewrite_enabled(self) -> bool:
        ...
        `thulr.rewrite` for the four measurements that justify it.
        """
    ...
    def stats_enabled(self) -> bool:
        ...
        is one line, and `thulr stats --forget` empties it. What makes
    ...
    def max_commits(self) -> int | None:
        ...
        import `thulr.ingest` to learn a number. Config sits under
        ...
        model imports behind `thulr --help`. The default stays where it
    ...
    def server_token_env(self) -> str | None:
        ...
        default someone can fall into: `thulr serve` says so out loud.
        """
    ...
    def connectors(self) -> list[ConnectorSpec]:
        ...
        Absent from `DEFAULT` deliberately. `thulr init` would
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        Returns:
            A deep copy in the `thulr.toml` section layout, so callers
