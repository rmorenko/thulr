"""The vocabulary of a workspace: what a `thulr.toml` can say.

The types a config document is made of, with no reading, writing or
validating in sight — those are `thulr.config`, `.document` and
...
class Provider(StrEnum):
    ...
    See `thulr.embed.remote` for what it buys and what it costs."""
...
class Repository:
    ...
    Attributes:
        ...
        remote: Clone url `thulr sync` keeps `path` up to date from, or
        ...
        source: Set to `connector` for a snapshot repository — one whose
            files `thulr sync` writes by fetching `urls` and
            committing them (see `thulr.snapshot`). Mutually exclusive
            with `remote`:
        ...
        ignore: Path globs this repo excludes, on top of the walker's
            ...
            design (see `thulr.ingest.walker._skip_dir`).
