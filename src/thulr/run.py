...
Split out of `thulr.pipeline` when that module reached four roles and
...
from thulr.config import Repository
from thulr.ingest import PARSE_ERROR, IndexState, Skip, WalkedFile, examine
from thulr.model import Chunk, Hit, Kind

log = logging.getLogger("thulr.pipeline")
...
what that looks like from the outside — `thulr index` printed
...
class Definition:
    ...
    What `thulr why` and the MCP tool of the same name both answer. It
...
@dataclass(frozen=True, kw_only=True)
class FileReport:
    """What the index knows about one file — `thulr explain` in data.
...
class IndexReport:
    ...
    def candidates(self) -> int:
        ...
        excluded by `ignore` — and a repository with a large `.thulr`
...
def _blaming(repo_id: str, rel_path: str) -> Iterator[None]:
    ...
    `THULR_DEBUG` still shows every frame.
