...
it is the reason `thulr shell` was worth building before this.
"""
...
from thulr.paths import make_index_dir
...
class Summary:
    """What the log knows, as `thulr stats` prints it.
    ...
    Attributes:
        ...
        p50_ms, p95_ms: Latency **as felt**, which is not the same as
            search latency and is much larger. Every `thulr search` is
            ...
            it is the strongest argument for `thulr shell`.
...
class SearchLog:
    ...
    def summary(self, *, top: int = 5) -> Summary:
        """Everything `thulr stats` reports, in one pass per question.
