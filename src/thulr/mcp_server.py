...
already, so no plugin has to exist. Point it at `thulr mcp`.
...
`build()` knows nothing about transport. `thulr mcp` runs it over
stdio, which is what an editor spawns; `thulr serve` mounts the same
...
from thulr.links import KIND_LABELS, OCCURRENCE_ORDER, LinkKind
...
def build(pipeline: Pipeline | None = None) -> FastMCP:
    ...
    engine = pipeline
    server: FastMCP = FastMCP(name="thulr", instructions=INSTRUCTIONS)
