...
Loading plugins is `thulr.ingest.plugins`; this module only defines
...
from thulr.ingest.ast.core import HAS_TREE_SITTER, SpanExtractor
from thulr.ingest.ast.sfc import Section, SectionSplitter
...
@dataclass(frozen=True, kw_only=True)
class LanguageSpec:
    """Everything thulr needs in order to index one language.
...
class LanguageRegistry:
    ...
    tidiness. A plugin has to `import thulr...` at module level to
    subclass or build what it registers. If the import of thulr ended
    ...
    def _ensure_plugins(self) -> None:
        ...
        with self._plugin_lock:
            ...
            # `register`, and a plugin importing thulr must not send us
            # back around this loop.
            self._pending_plugins = False
            from thulr.ingest.plugins import load_plugins
...
BUILTIN_LANGUAGES: tuple[LanguageSpec, ...] = (
    ...
    LanguageSpec(
        ...
        # actually written in. See `thulr.ingest.ast.sfc`.
        grammar=GrammarSpec(module="tree_sitter_html", getter="language"),
...
"""The languages thulr ships with. Only the ARCH §1 corpus is listed on
