...
of any size runs in a small child process, and `thulr.ingest.blame` for
...
from thulr.ingest.git_state import GIT_TIMEOUT, GitCommandError, decode_path, run_git
from thulr.links import Link, LinkKind
...
def commit_chunks(commits: list[Commit], *, repo: str) -> list[Chunk]:
    ...
    return [
        Chunk(
            ...
            # `thulr why` has something to join blame links on.
            symbol=commit.short,
            node_type="commit",
            start_line=1,
...
def blame_map(root: Path, rel_paths: Sequence[str]) -> dict[str, dict[int, str]]:
    ...
    A batch of any size runs in a child process (see `thulr.ingest.blame`
