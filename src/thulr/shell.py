"""`thulr shell`: ask many questions of one loaded model.

A `thulr search` spends most of its seconds before it searches
...
are `thulr search`'s, because this is another adapter over the same
...
from thulr.ui import console, render_hits
...
_HELP = """\
Type a question to search. Flags are the ones `thulr search` takes:
...
def run(pipeline: Pipeline, *, history_dir: Path) -> None:
    ...
    session.out.print(
        f"thulr [bold]{Config().name}[/] — model loaded once, ask away. :help for the rest."
    )
    while True:
        try:
            line = prompt.prompt("thulr> ").strip()
