...
One rule: **rich output only on a terminal.** `thulr search x | grep`
and `> hits.txt` have to keep working, and a table drawn with box
characters is not something to grep. A pipe, a file, a CI log,
`NO_COLOR` and `THULR_PLAIN` all get the one-line-per-hit format this
...
answer it. `thulr shell` is where the whole thing gets read."""
...
def wants_rich(target: Console | None = None) -> bool:
    ...
    set (the informal standard), or `THULR_PLAIN` is set (ours, for
