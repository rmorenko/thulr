"""Thulr — a learning CLI that semantically indexes a developer workspace.

The entry point lives in `thulr.cli`. What the tool does and what it
...
Two callers, two answers. `thulr search` is run by a person, so the
CLI keeps writing sentences to stderr and turns none of this on: a log
line is not an interface for somebody watching a terminal. `thulr
