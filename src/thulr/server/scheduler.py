...
The tick does what `thulr sync` does, every `[server] interval`
...
if TYPE_CHECKING:  # pragma: no cover - import-time only, for annotations
    from thulr.pipeline import IndexReport

log = logging.getLogger(__name__)


def sync_and_index(app: FastAPI) -> dict[str, Any]:
    """One full cycle: pull, materialize, index. What `thulr sync` does.
    ...
    from thulr.server.api import run_detail
...
class Ticker:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(target=self._loop, name="thulr-ticker", daemon=True)
        self._thread.start()
