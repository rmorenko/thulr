...
from thulr.server.scheduler import sync_and_index
...
def _stats_panel(app: FastAPI) -> str:
    ...
    of a page. So: the same numbers `thulr stats` prints, aggregated,
    ...
    # Not the CLI's label, deliberately. `thulr stats` says "per
...
<p class="note">Everyone's questions on this server, together — nobody's
 ...
 <code>thulr stats --forget</code>.</p>
...
<title>thulr — {html.escape(config.name)}</title><style>{_STYLE}</style></head>
<body>
<h1>thulr <span class="note">— {html.escape(config.name)}</span></h1>
...
def mount_admin(app: FastAPI, guarded: list[Any]) -> None:
    ...
    """
    from thulr.server.api import Busy
    ...
    def admin_add_repo(
        ...
        Writes through `Config`, exactly as `thulr add-repo` does, so
