...
is `thulr search --repo`. Two interfaces over one engine stay honest
...
from thulr.ingest import NotAGitRepositoryError
from thulr.model import Kind, SearchFilter
from thulr.server.admin import mount_admin
from thulr.server.metrics import CONTENT_TYPE, Meter, Metrics
from thulr.server.scheduler import mount_scheduler
...
# nothing imports `thulr.server` except `thulr serve`, which checks
# for the extra first.
...
def _mount_mcp(app: FastAPI) -> None:
    ...
    code. `thulr mcp` runs the server over stdio for an editor that
