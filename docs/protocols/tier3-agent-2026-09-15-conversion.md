...
_2026-09-15 01:11, wsindex `18dc086`, 12 tasks, 69 min._

## Conditions

- One real agent, the same on both sides, given a real issue and the
  repository **at the commit before the fix**.
- Both keep every ordinary tool; the treatment adds wsindex over MCP
...
- Tools the runs actually had, as the agent reported at startup: **9** with wsindex, **6** without — `Bash`, `Edit`, `Glob`, `Grep`, `Read`, `Write`, `mcp__wsindex__refs`, `mcp__wsindex__search`, `mcp__wsindex__why`.

## The gate, declared before the run

With wsindex the agent lands on the same lines at least as often,
and spends fewer tokens doing it.

## Results

| | with wsindex | without |
| --- | ---: | ---: |
