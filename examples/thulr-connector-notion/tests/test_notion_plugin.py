...
import pytest
from thulr.connectors import ConnectorError, ConnectorSpec, DocumentNotFound

from thulr_connector_notion import NotionConnector, page_id
...
def test_a_page_becomes_markdown(api: dict[str, dict[str, object]]) -> None:
    ...
    api["children"] = {
        "results": [
            ...
            block("code", "uv run thulr index", language="bash"),
            block("divider"),
        ],
        "has_more": False,
    }
...
def test_formatting_survives_the_trip(api: dict[str, dict[str, object]]) -> None:
    ...
    assert NotionConnector(SPEC).fetch(PAGE_URL).text == (
        "Run `thulr sync`** nightly**[docs](https://x.invalid)"
    )
