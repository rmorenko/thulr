...
plugin = pytest.importorskip(
    "thulr_lang_lua",
    reason="run `uv pip install -e examples/thulr-lang-lua` first",
)
LUA = plugin.LUA
...
def test_lua_is_not_a_built_in() -> None:
    # The reason this example is Lua: a language thulr ships with would
    # claim the same suffix, and the loader would skip the plugin.
    from thulr.ingest.languages import BUILTIN_LANGUAGES
