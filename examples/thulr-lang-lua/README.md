...
Lua support for [thulr](../../README.md), and the worked example every
language plugin can be copied from.

Install it and `.lua` files start being indexed — every function as its
own chunk, however it is spelled. No change to thulr itself:

```bash
uv pip install -e examples/thulr-lang-lua
uv run thulr index
uv run thulr search "how does it greet someone" --lang lua
```
...
**Why Lua?** Because it is deliberately *not* a language thulr ships
...
**1. A dependency on the grammar.** thulr ships grammars for the
...
that makes an installed package visible to thulr:
...
into `thulr_lang_lua-0.1.0.dist-info/entry_points.txt` when the package
is installed, and thulr reads it back with `importlib.metadata`. A
...
**3. A `LanguageSpec`.** Everything thulr needs to know:
...
thulr refuses a spec with one and not the other, because an extractor
...
twice. The helpers come from `thulr.ingest.ast`:
...
- **Name the grammar, do not import it.** `GrammarSpec` takes a module
  name so the spec stays declarable when the grammar is missing; thulr
...
```bash
uv pip install -e examples/thulr-lang-lua
uv run pytest examples/thulr-lang-lua/tests
```
