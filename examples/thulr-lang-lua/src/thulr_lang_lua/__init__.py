"""Lua support for thulr, written against the published plugin surface.

Everything this package imports comes from two places:

- `thulr.ingest` — the specification: `LanguageSpec`, `GrammarSpec`.
- `thulr.ingest.ast` — the toolkit an extractor is written with:
...
Nothing private, and no edit to thulr itself. That is the claim the
plugin system makes, and this package is where it gets tested.

Lua on purpose: it is deliberately *not* one of the languages thulr
...
from thulr.ingest.ast import Span, line_span, mark_covered, symbol_name
...
def lua_spans(root: Node, lines: list[str], covered: list[bool]) -> list[Span]:
    ...
    Signature fixed by `thulr.ingest.SpanExtractor`. `lines` goes
...
LUA = LanguageSpec(
    ...
    # somehow absent, thulr falls back to text windows for `.lua` files
